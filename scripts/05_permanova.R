...
adonis_bio_global <- adonis2(dist_bio ~ type + classes + sp + site_loc + site, data = metadata_bio)
adonis_classes     <- adonis2(dist_bio ~ classes, data = metadata_bio)
...
adonis_sub_global   <- adonis2(dist_sub ~ classes + sp + site_loc + site, data = metadata_sub)
adonis_sub_classes   <- adonis2(dist_sub ~ classes, data = metadata_sub)
...
results <- bind_rows(
  ...
  extract_results(adonis_classes,       "classes (biofilm + subcuticle)"),
  ...
  extract_results(adonis_sub_classes,   "classes (subcuticle only)"),

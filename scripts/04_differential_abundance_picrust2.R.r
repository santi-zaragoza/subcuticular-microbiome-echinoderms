...
# Parameters
sample_types <- c("sedimento", "subcuticle")  # tipos de muestra a incluir
...
for (submetodo in unique(resultados$method)) {
  resultado_sub <- resultados %>% filter(method == submetodo)
  feature_with_p_0.05 <- resultado_sub %>% filter(p_adjust < 0.05)
  ...
  if (nrow(feature_selected) > 0) {
    ...
    row_labels <- pathway_annotation(pathway = "MetaCyc", daa_results_df = resultado_sub, ko_to_kegg = FALSE) %>%
